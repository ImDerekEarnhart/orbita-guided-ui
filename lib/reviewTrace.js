...
function rowToQuestion(row) {
  ...
  return {
    ...
    status: row.status,
    question_class: row.question_class || row.provenance_json?.question_class || "generalization",
    why_allowed: row.why_allowed,
    why_blocked: row.why_blocked,
    what_would_make_it_admissible: row.what_would_make_it_admissible || null,
    evidence_refs: row.evidence_refs_json || [],
    counterexample_refs: row.counterexample_refs_json || [],
    trace_event_refs: row.trace_event_refs_json || [],
    ...
    programme_state_snapshot_id: row.programme_state_snapshot_id || null,
...
async function createTraceEventOnce(userId, graphId, input = {}) {
  const eventType = normalizeTraceType(input.event_type);
  const sourceType = text(input.source_type, 80);
  const sourceRefId = text(input.source_ref_id, 240);
  if (sourceType && sourceRefId) {
    const { rows } = await db.query(
      `SELECT * FROM research_trace_events
       WHERE user_id = $1 AND graph_id = $2 AND event_type = $3
         AND source_type = $4 AND source_ref_id = $5
       ORDER BY created_at DESC
       LIMIT 1`,
      [userId, graphId, eventType, sourceType, sourceRefId]
    );
    if (rows[0]) return rowToTraceEvent(rows[0]);
  }
  return createTraceEvent(userId, graphId, { ...input, event_type: eventType, source_type: sourceType, source_ref_id: sourceRefId });
}
    ...
    const { rows } = await db.query(
      `INSERT INTO admissible_questions
         (graph_id, user_id, question_id, question_text, status, why_allowed, why_blocked,
          ...
          question_class, what_would_make_it_admissible, trace_event_refs_json,
          programme_state_snapshot_id, review_needed)
       ...
       ON CONFLICT (graph_id, user_id, question_id) DO UPDATE SET
         ...
         what_would_make_it_admissible = EXCLUDED.what_would_make_it_admissible,
         evidence_refs_json = EXCLUDED.evidence_refs_json,
         counterexample_refs_json = EXCLUDED.counterexample_refs_json,
         trace_event_refs_json = EXCLUDED.trace_event_refs_json,
         ...
         programme_state_snapshot_id = EXCLUDED.programme_state_snapshot_id,
      ...
      [
        ...
        text(question.question_class, 80) || "generalization",
        text(question.what_would_make_it_admissible, 2000),
        JSON.stringify(jsonArray(question.trace_event_refs)),
        question.programme_state_snapshot_id || question.provenance?.source_snapshot_id || null,
        question.review_needed !== false,
      ]
    );
