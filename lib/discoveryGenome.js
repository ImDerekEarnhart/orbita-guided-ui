...
function buildTournamentRevealReceipt({ tournamentId, manifestHash, reveal }) {
  return canonicalize({
...
async function markTournamentRevealed(userId, tournamentId, input) {
  const expectedManifestHash = requireHash(input?.expected_manifest_hash, "expected_manifest_hash");
  if (!input?.reveal || typeof input.reveal !== "object" || Array.isArray(input.reveal)) {
    throw new Error("reveal must be an object");
  }
  const normalizedReveal = canonicalize(input.reveal);
  const receipt = buildTournamentRevealReceipt({
    tournamentId,
    manifestHash: expectedManifestHash,
    reveal: normalizedReveal,
  });
  const revealHash = hashJson(receipt);
  if (input?.expected_reveal_hash) {
    const expectedRevealHash = requireHash(input.expected_reveal_hash, "expected_reveal_hash");
    if (!crypto.timingSafeEqual(Buffer.from(expectedRevealHash), Buffer.from(revealHash))) {
      throw new Error("Tournament reveal hash mismatch");
    }
  }
  ...
  try {
    ...
    if (!tournaments.length) throw new Error("Tournament not found");
    const tournament = tournaments[0];
    if (!["frozen", "revealed", "running"].includes(tournament.status)) {
      throw new Error("Only a frozen tournament can be marked revealed");
    }
    requireHashMatch(expectedManifestHash, tournament.manifest_hash, "Tournament");
    if (tournament.reveal_hash && tournament.reveal_hash !== revealHash) {
      throw new Error("Tournament reveal hash mismatch");
    }
    const { rows } = await client.query(
      `UPDATE discovery_tournaments
       SET status = CASE WHEN status = 'frozen' THEN 'revealed' ELSE status END,
           revealed_at = COALESCE(revealed_at, NOW()),
           reveal_json = COALESCE(reveal_json, $1::jsonb),
           reveal_hash = COALESCE(reveal_hash, $2),
           updated_at = NOW()
       WHERE id = $3 AND user_id = $4
       RETURNING *`,
      [JSON.stringify(normalizedReveal), revealHash, tournamentId, userId]
    );
    await client.query("COMMIT");
    return { ...rows[0], reveal_hash: revealHash };
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
    throw err;
  } finally {
...
async function recordTournamentResult(userId, tournamentId, entryId, input) {
  ...
  try {
    ...
    if (!["revealed", "running"].includes(current.tournament_status) || current.verdict !== "pending") {
      throw new Error("Pending revealed tournament entry not found");
    }
    const { rows } = await client.query(
    ...
    if (!rows.length) throw new Error("Pending revealed tournament entry not found");
    await client.query(
